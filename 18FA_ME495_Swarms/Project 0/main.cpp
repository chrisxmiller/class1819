...
int num_robots = 102; //number of robots running
...
void setup_positions()
{
	...
	int x = arena_width/2;
	int y = arena_height/2 -(vertical_separation*rows)/2;
	//Sorry for the hard coded values - I had to submit my GRFP and ran low on
	//time and needed *some* sleep to function. Hope this is okay as a one-time
	//thing!
	int offsets [11] = {100, 80, 60, 40, 20, 0, 20, 40, 60, 80, 100};
	int numbots [11] = {7,8,9,10,11,12,11,10,9,8,7};
	int top = 1;
	//Set their angle to 0 degrees
	double t = 0;
	for (int i = 0; i < rows; i++){
		for(int j = 0; j < numbots[i]; j++){
			x = j * horizontal_separation + offsets[i] + arena_width/2
															- (columns*horizontal_separation)/2;
			robots[k] = new mykilobot();
			robots[k]->robot_init(x,y,t);
			k++;
		}
		y = y + vertical_separation;
	}
}
